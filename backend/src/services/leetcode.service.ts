...
const fetchLeetCodeProfile = async (username: string) => {
    ...
    try {
        ...
        if(response){
            ...
            if(!userLCprofile){
                const contests_attended = response.data.data.userContestRankingHistory.filter((ele:any)=>{return ele.attended});
...
async function saveLeetCodeContests(topTwoContests: any[]) {
    try {
        ...
        if(topTwoContests.length<=2){
        ...
        else {contestsToInsert = topTwoContests.map(ele => ({
            ...
            link: `https://leetcode.com/contest/${ele.contest.title.toLowerCase().replace(/\s+/g, '-')}/`
        }));}
